<template>
    <div>
        <el-page-header @back="goBack" content="修改演出信息" style="margin-left: 20%;margin-top: 10px;">
        </el-page-header>
...
<script>
    export default {
        name: "ChangeShow",
        beforeMount(){
            this.performId = this.$route.query.uid;
        },
        mounted(){
            this.getCity();
            this.getTypes();
        ...
        methods: {
            onSubmit(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios({
                            method:"post",
                            url:"http://118.31.7.87:8080/perform/modify",
                            headers:{
            ...
            getAll(){
                this.$axios.post("http://118.31.7.87:8080/perform/detail/"+this.performId
                ).then(res =>{
                    if(res.data.msg === '成功'){
                        this.item = res.data.data;
                        console.log(this.item);
                        this.form.title = this.item.title;
                        this.form.city = this.item.city.id;
                        this.form.type = this.item.category.id;
                        this.form.desc = this.item.content;
                        if(this.item.subclass.id != null){this.getSmallType();}
                        if(this.item.location.id != null){this.getPosition();}
                        this.form.sType = this.item.subclass.id;
                        this.form.position = this.item.location.id;
                        this.form.art = this.item.artist.id;
                        this.form.viewNotice = this.item.viewNotice;
                        this.form.ticketNotice = this.item.ticketNotice;
                        this.form.file = this.item.cover;
                        this.fileList =[{name:'1.jpg',src:this.item.cover}];
                    }else{
